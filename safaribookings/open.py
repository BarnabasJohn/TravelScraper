...
for index, (key, value) in enumerate(company.items()):
    ...
    url = f'https://www.safaribookings.com/{value['url']}/t38636'
    ...
    for offer in offers:
        keshi[key] = {}
        keshi[key] = offer.get_text()


with open('Aberdare/data.json', 'w') as json_file:
        json.dump({"Company": keshi}, json_file, indent=4)

'''
import json

# Function to recursively clean spaces in all string values in a dictionary
...
with open('Aberdare/data.json', 'r') as json_file:
...
with open('Aberdare/data.json', 'w') as cleaned_file:
